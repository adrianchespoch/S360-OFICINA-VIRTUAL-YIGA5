...
export const useDeleteIVA = () => {
  const queryClient = useQueryClient();
  const setIsGlobalLoading = useUiStore.getState().setIsGlobalLoading;

  return useMutation({
    mutationFn: deleteIVA,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['i-vas'] });
      ToastWrapper.success('IVA eliminado correctamente');
    },
    onError: error => {
...
export const deleteIVA = async (id: number) => {
  const setIsGlobalLoading = useUiStore.getState().setIsGlobalLoading;
  setIsGlobalLoading(true);

  return remove<IVA>(`/iva/${id}/`, true);
};
